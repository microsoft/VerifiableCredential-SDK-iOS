...
class PostPresentationRequestTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            postPresentationResponseOperation = try PostPresentationResponseOperation(withUrl: self.expectedUrl, withBody: testRequestBody, urlSession: urlSession)
        } catch {
            print(error)
        }
    }
    ...
    func testSuccessfulPostOperation() {
        ...
        postPresentationResponseOperation.fire().done { result in
            print(result)
            switch result {
            case .success(let response):
                print(response)
                XCTAssertEqual(response as! String, self.expectedHttpResponse)
            case .failure(_):
                XCTFail()
            }
            expec.fulfill()
